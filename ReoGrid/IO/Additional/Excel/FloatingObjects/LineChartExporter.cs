...
using ChartType = unvell.ReoGrid.Chart.LineChart;

namespace unvell.ReoGrid.IO.Additional.Excel.FloatingObjects
{
    class LineChartExporter : DrawingObjectExporterBase
    {
...
#warning  пропущен <mc:AlternateContent xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006">
            ...
            var catAx = CreateCatAx(
                ...
                chart.HorizontalAxisInfoView.ReverseOrderOfCategories
                );
            var valAx = CreateValAx(valId, axId, chart.VerticalAxisInfoView.ReverseOrderOfCategories);
        ...
        private static CT_CatAx CreateCatAx(CT_UnsignedInt id, CT_UnsignedInt crossId, bool delete, AxisTextDirection direction, bool order)
        {
            ...
            {
                axId = id,
                scaling = new CT_Scaling { orientation = new CT_Orientation { val = order ? ST_Orientation.maxMin : ST_Orientation.minMax} },
...
#warning <c:crosses val="autoZero"/> пропущен
        ...
        private static CT_ValAx CreateValAx(CT_UnsignedInt id, CT_UnsignedInt crossId, bool order)
        {
            return new CT_ValAx
            {
                axId = id,
                ...
                txPr = new CT_TextBody
                {
                    bodyPr = new CT_TextBodyProperties
                    {
                        rot = -60000000, rotSpecified = true,
...
#warning <c:crosses val="autoZero"/> пропущен
        ...
        private static CT_LineSer[] CreateLineSerArray(CT_ChartSpace space, ChartType chart)
        {
            var count = (uint)chart.DataSource.SerialCount;
            var result = new CT_LineSer[count];
        ...
        private static CT_LineSer CreateLineSer(CT_ChartSpace space, ChartType chart, uint index)
        {
            ...
            return new CT_LineSer
            {
                ...
                spPr = new CT_ShapeProperties
                {
                    ln = new CT_LineProperties
                    {
                        w = 28575 , wSpecified = true,
                        cap = ST_LineCap.rnd, capSpecified =  true,
